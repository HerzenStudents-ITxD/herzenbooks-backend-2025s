...
public class FilterAdminOrdersCommand : IFilterAdminOrdersCommand
{
    private readonly IOrderRepository _repository;
    private readonly IAccessValidator _accessValidator;

    public FilterAdminOrdersCommand(IOrderRepository repository, IAccessValidator accessValidator)
    {
        _repository = repository;
        _accessValidator = accessValidator;
    }

    public async Task<OperationResultResponse<List<OrderResponse>>> ExecuteAsync(FilterOrdersRequest request)
    {
        if (!await _accessValidator.IsAdminAsync())
            return new OperationResultResponse<List<OrderResponse>>(null, new List<string> { "Only admins can access this resource." });

        var orders = await _repository.FilterOrdersAsync(
